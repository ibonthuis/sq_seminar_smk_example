...
DATA_DIR = config["data_dir_single_dataset"]
BASE_OUTPUT_DIR = config["output_dir"]
METADATA_SINGLE_DATASET = config["metadata_single_dataset"]
EXPRESSION_SINGLE_DATASET = config["expression_single_dataset"]
VARIABLE_TO_VISUALIZE = config["visualisation_var"]

FILTERED_METADATA_SINGLE_DATASET_RDATA = os.path.join(BASE_OUTPUT_DIR, "filtered_metadata_single_dataset.RData")
PCA_PLOT_SINGLE_DATASET_PDF = os.path.join(BASE_OUTPUT_DIR, "figures", "pca_plot_single_dataset.pdf")


rule all:
    input:
        FILTERED_METADATA_SINGLE_DATASET_RDATA,
        PCA_PLOT_SINGLE_DATASET_PDF

rule format_metadata:
    """
    Inputs
    ------
    METADATA_SINGLE_DATASET:
    ...
    """
    input:
        metadata = METADATA_SINGLE_DATASET
    output:
        filtered_metadata_single_dataset = FILTERED_METADATA_SINGLE_DATASET_RDATA
    message:
    ...
    shell:
        ...
        Rscript {params.bin}/filter_metadata.R \
            -m {input.metadata} \
            -o {output.filtered_metadata_single_dataset} 
        """
...
rule perform_dimensionality_reduction:
    ...
    """
    input:
        metadata = FILTERED_METADATA_SINGLE_DATASET_RDATA, \
        expression = EXPRESSION_SINGLE_DATASET
    output:
        pca_plot_pdf = PCA_PLOT_SINGLE_DATASET_PDF
    message:
